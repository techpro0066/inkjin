...
<script>
  ...
  function renderUserRequestDetail(req) {
    ...
    var messageArtistHtml = req.chatUrl
      ? '<a href="' + escapeHtml(req.chatUrl) + '" class="mt-2 inline-flex items-center gap-1.5 rounded-full bg-primary/10 text-primary text-sm font-semibold px-3.5 py-1.5 hover:bg-primary/15 transition-colors"><span class="material-symbols-outlined text-[18px]">chat</span> Message artist</a>'
      : (profileLink ? '<p class="mt-1">' + profileLink + '</p>' : '');

    left.innerHTML =
      '<div class="flex items-center gap-4"><div class="w-14 h-14 rounded-full bg-primary flex items-center justify-center flex-shrink-0"><span class="text-white text-lg font-bold">' + escapeHtml(req.artistInitials) + '</span></div><div><p class="font-bold text-lg text-on-surface">' + escapeHtml(req.artistName) + '</p>' + messageArtistHtml + (req.chatUrl && profileLink ? '<p class="mt-1.5">' + profileLink + '</p>' : '') + '</div></div>' +
