...
@endphp
<div class="portfolio-card bg-white rounded-2xl border border-outline-variant/20 overflow-hidden shadow-sm" data-portfolio-id="{{ $portfolio->id }}" data-is-active="{{ $portfolio->is_active ? '1' : '0' }}" data-sort-order="{{ (int) ($portfolio->sort_order ?? 0) }}">
  <div class="aspect-square bg-surface-container-high rounded-t-2xl relative overflow-hidden">
    <button type="button" class="portfolio-drag-handle" title="Drag to reorder" aria-label="Drag to reorder">
      <span class="material-symbols-outlined">drag_indicator</span>
    </button>
    <img src="{{ asset($portfolio->image) }}" alt="" class="w-full h-full object-cover pointer-events-none">
    @if ($fromInstagram)
