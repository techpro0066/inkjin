...
<main class="main-content flex-1 min-h-screen">
    <form id="personalPageForm" class="p-6 md:p-10 lg:p-12 max-w-6xl" enctype="multipart/form-data">
      ...
      @if ($showStripeIdentity ?? false)
        <div id="stripeIdentitySection" class="bg-surface-container-low rounded-2xl p-5 md:p-6 mb-6 border border-outline-variant/20">
          <div class="flex flex-wrap items-start justify-between gap-4 mb-4">
            <div class="flex items-start gap-3">
              ...
              <div>
                <h3 class="font-bold text-on-surface">Identity verification</h3>
                <p class="text-sm text-on-surface-variant mt-1 max-w-2xl">
                  Verify your identity with a passport or government ID. This is required for payouts and keeps your account secure.
                </p>
              </div>
            </div>
            @if ($stripeIdentityComplete ?? false)
              <span class="inline-flex items-center gap-1 rounded-full bg-emerald-50 text-emerald-800 border border-emerald-200 px-3 py-1 text-xs font-semibold">
                <span class="material-symbols-outlined text-sm">check_circle</span> Verified
              </span>
            @else
              <span id="stripeIdentityStatusBadge" class="inline-flex items-center gap-1 rounded-full bg-amber-50 text-amber-900 border border-amber-200 px-3 py-1 text-xs font-semibold">
                <span class="material-symbols-outlined text-sm">pending</span> Action required
              </span>
            @endif
          </div>

          @if ($stripeConnectConfigured ?? false)
            <div id="stripeIdentityMount" class="min-h-[360px] rounded-xl bg-white overflow-hidden border border-outline-variant/20"></div>
            <p id="stripe_identity_error" class="text-error text-xs mt-2 hidden"></p>
            <p id="stripeIdentityHint" class="text-on-surface-variant text-xs mt-3 {{ ($stripeIdentityComplete ?? false) ? 'hidden' : '' }}">
              Complete your personal details and upload your identity document below.
            </p>
          @else
            <div class="rounded-xl border border-red-200 bg-red-50 text-red-800 px-4 py-3 text-sm">
              Stripe is not configured. Contact support if you need help with identity verification.
            </div>
          @endif
        </div>
      @endif
  ...
  const sessionUrl = @json(route('personal-page.stripe.identity.session'));
  const statusUrl = @json(route('personal-page.stripe.identity.status'));
  const stripeConnectLocale = @json($stripeConnectLocale ?? 'en-US');
  let identityMounted = false;
  ...
  async function refreshIdentityStatus() {
    ...
    const complete = !data.identity_verification_due;
    const badge = document.getElementById('stripeIdentityStatusBadge');
    const hint = document.getElementById('stripeIdentityHint');
    if (complete) {
  ...
  async function mountStripeIdentity() {
    const container = document.getElementById('stripeIdentityMount');
    if (!stripeConfigured || !publishableKey || !container || identityMounted) {
      return;
    }

    container.innerHTML = '<p class="text-sm text-on-surface-variant p-6">Loading identity verification…</p>';
    ...
    try {
      ...
      accountOnboarding.setOnStepChange(() => {
        setTimeout(refreshIdentityStatus, 800);
      });

      container.innerHTML = '';
      container.appendChild(accountOnboarding);
      identityMounted = true;
    } catch (err) {
      container.innerHTML = '';
      const errEl = document.getElementById('stripe_identity_error');
      if (errEl) {
        errEl.textContent = err.message || 'Could not load identity verification.';
