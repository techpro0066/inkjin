...
<body class="bg-surface text-on-surface min-h-screen">
        ...
        <div class="flex flex-col lg:flex-row gap-6">
          <div class="lg:w-[340px] lg:order-2">
            <div class="bg-white rounded-2xl border border-outline-variant/20 p-5 lg:sticky lg:top-24">
              ...
              <div class="bg-surface-container-low rounded-xl p-3"><p class="text-xs font-bold text-on-surface-variant uppercase tracking-wider mb-2">Due at Studio</p><div class="space-y-1.5 text-sm"><div class="flex justify-between"><span class="text-on-surface-variant">Remaining Balance (est.)</span><span class="font-semibold" id="payBalance">—</span></div><p class="text-xs text-on-surface-variant italic mt-1">If you get this design as-is (original size, no modifications), expect to pay the minimum. Final price confirmed by the artist based on size, placement, and any customizations.</p></div></div>
            </div>
          </div>
          <div class="flex-1 lg:order-1">
            ...
            <button type="button" id="btnConfirmKlarna" class="hidden w-full py-4 rounded-xl font-bold text-[#17120F] bg-[#FFB3C7] hover:bg-[#FF9CB8] transition-colors text-base shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">Pay with Klarna</button>
  ...
  <script>
    ...
    function bookingPhone() {
      return (window.InkjinPhoneCountry && window.InkjinPhoneCountry.getFullPhone('bd')) || '';
    }

    function bookingPhoneIso() {
      return (window.InkjinPhoneCountry && window.InkjinPhoneCountry.getSelectedIso('bd')) || '';
    }

    function setBookingPhone(e164) {
      if (window.InkjinPhoneCountry) {
        window.InkjinPhoneCountry.setFullPhone('bd', e164 || '');
      }
    }
    ...
    function updateIrisTabVisibility() {
      ...
      var showIris = isGreekClientPhone(bookingPhone());
    ...
    function updatePaymentSummary() {
      ...
      var sizeLabel = requestedSize || @json($tattoo->sizeLabel());
    ...
    async function persistBookingRecord(paymentIntentId, bookingPayloadOverride) {
      var response = await fetch('/api/public/confirm-booking-payment', {
        ...
        body: JSON.stringify({
          ...
          booking_payload: bookingPayloadOverride || buildBookingPayload()
        })
      });
      var data = await response.json();
    ...
    function collectBookingDraftState() {
      ...
      return {
        ...
        bdPhone: bookingPhone(),
        bdPhoneCountry: bookingPhoneIso(),
    ...
    function applyBookingDraftFromSession(state) {
      ...
      setBookingPhone(String(state.bdPhone || ''));
      if (state.bdPhoneCountry) {
        $('#bdPhoneCountry').val(String(state.bdPhoneCountry)).trigger('change');
      }
    ...
    async function nextReg() {
      ...
      if (currentRegIndex === 2) {
        var phoneVal = bookingPhone();
        var nationalVal = String($('#bdPhone').val() || '').trim();
        if (!nationalVal) {
          setRegError('bdPhone', 'bdPhoneError', 'This field is required.');
          return;
        }
        if (!$('#bdPhoneCountry').val()) {
    ...
    $('#bdPhone, #bdPhoneCountry').on('input change', function() {
      clearRegError('bdPhone', 'bdPhoneError');
      updateIrisTabVisibility();
      updatePaymentSummary();
    });
    ...
    $(document).on('click', '.js-cc-tattoo-slot', function() {
      ...
      $(document).on('input change', '#bdName, #bdEmail, #bdPhone, #bdPhoneCountry, #bdOtpEmail, #bdOtpCode, .js-question-input, .js-select2-question, .js-question-toggle', scheduleBookingDraftSave);
  ...
  <script>
    ...
    function klarnaPendingStorageKey() {
      return 'inkjin_klarna_pending:' + bookingArtistUsername + ':' + bookingTattooSlug;
    }

    function syncKlarnaFieldsFromBooking() {
      var nameEl = document.getElementById('inputKlarnaName');
      var emailEl = document.getElementById('inputKlarnaEmail');
      var name = String(bookingConnectedName || $('#bdName').val() || $('#inputCardName').val() || '').trim();
      var email = String(bookingConnectedEmail || $('#bdEmail').val() || '').trim();
      if (nameEl && !String(nameEl.value || '').trim() && name) nameEl.value = name;
      if (emailEl && !String(emailEl.value || '').trim() && email) emailEl.value = email;
    }

    window.checkoutKlarnaConfig = {
      currency: 'eur',
      getStripe: function () {
        if (!stripe && typeof mountStripeElements === 'function') mountStripeElements();
        return stripe;
      },
      getAmountCents: function () {
        return Math.round((getDueNow() || 0) * 100);
      },
      isPolicyAccepted: function () {
        return !!$('#agreePolicy').is(':checked');
      },
      onReturnStart: function () {
        goToStep(5);
        $('#processingView').removeClass('hidden');
        $('#confirmationCalendar, #confirmationManaged').addClass('hidden');
        $('#processingText').text('Confirming your Klarna payment...');
      },
      beforeRedirect: async function (billing) {
        syncKlarnaFieldsFromBooking();
        try {
          sessionStorage.setItem(klarnaPendingStorageKey(), JSON.stringify({
            booking_payload: buildBookingPayload(),
      ...
      getClientSecret: async function (billing) {
        var response = await fetch('/api/public/create-booking-payment-intent', {
          ...
          body: JSON.stringify({
            artist_username: bookingArtistUsername,
            tattoo_slug: bookingTattooSlug,
            cardholder_name: String((billing && billing.name) || '').trim() || 'Klarna',
            phone: bookingPhone()
          })
        });
        var data = await response.json();
        if (!response.ok || !data || !data.client_secret) {
          throw new Error((data && data.message) || 'Unable to initialize Klarna payment.');
        }
        return data.client_secret;
      },
      onSuccess: async function (paymentIntentId) {
        ...
        $('#confirmationCalendar, #confirmationManaged').addClass('hidden');
        $('#processingText').text('Confirming your Klarna payment...');
        var payloadOverride = null;
        try {
          var raw = sessionStorage.getItem(klarnaPendingStorageKey());
          if (raw) {
            var parsed = JSON.parse(raw);
            payloadOverride = parsed && parsed.booking_payload ? parsed.booking_payload : null;
          }
        } catch (e) {}
        try {
          var savedBooking = await persistBookingRecord(paymentIntentId, payloadOverride);
          try { sessionStorage.removeItem(klarnaPendingStorageKey()); } catch (e) {}
          if (typeof window.clearBookingDraftSession === 'function') {
            window.clearBookingDraftSession();
          }
          $('#confRef').text(savedBooking.booking_reference || ('#INK-' + String(paymentIntentId || '').replace('pi_', '').slice(-6).toUpperCase()));
          ...
          $('#formError').removeClass('hidden').text(error.message || 'Payment succeeded but booking could not be saved.');
          var klarnaErr = document.getElementById('klarnaFormError');
          if (klarnaErr) {
            klarnaErr.textContent = error.message || 'Payment succeeded but booking could not be saved.';
            klarnaErr.classList.remove('hidden');
          }
        }
      },
      onError: function (error) {
        ...
        $('#formError').removeClass('hidden').text(error.message || 'Klarna payment failed.');
      }
    };

    document.getElementById('tabPayKlarna')?.addEventListener('click', syncKlarnaFieldsFromBooking);
  </script>
  <script>
