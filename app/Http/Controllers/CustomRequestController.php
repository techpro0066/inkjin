...
class CustomRequestController extends Controller
{
    ...
    public function submitCustomRequest(Request $request): JsonResponse
    {
        ...
        $requestUser->syncHearAboutUs($payload['referral_source'] ?? null);
    ...
    private function prefillAnswersFromPortfolio(mixed $portfolioId, int $artistUserId, array $questions): array
    {
        ...
        // Text fields as before; image prefill is reference_image only (not placement/coverup).
    ...
    private function adaptPortfolioPrefillToQuestionWidget(
        ...
        if (in_array($questionType, $imageQuestionTypes, true) || $widgetType === 'image') {
            if ($widgetType !== 'image' || $questionType !== 'reference_image') {
                return null;
            }
